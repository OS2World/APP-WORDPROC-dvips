...
Boolean mfmode_option ;       /* set mfmode from command line? */
Boolean oname_option ;	      /* set output name from command line? */
...
case 'u' :
		  ...
		  if (PSname[0] == '+')
		     ...
		     psmapfile = strdup(PSname) ; /* a cute small memory leak (j
ust as in 'p' option handling in resident.c) */
	       }
	       break ;
case 'h' : case 'H' :
	       if (*p == 0 && argv[i+1])
